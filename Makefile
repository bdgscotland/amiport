...
help:
	...
	@echo "  build-libjpeg    Cross-compile libjpeg 9f (JPEG image decoder, integer DCT only)"
	@echo "  build-libcurl    Cross-compile libcurl 8.11.1 HTTP-only (no TLS, no proxy, no auth)"
	...
	@echo "  test-libjpeg     Run lib/libjpeg tests via vamos -C 68040 -s 1024 -m 4096 (18/18)"
	@echo "  test-libcurl     Run lib/libcurl tests via vamos -C 68040 -s 4096 -m 8192 (21/21)"
	...
	@echo "  build-keyinject  Build KeyInject (keystroke injector for interactive tests)"
...
build-libtommath:
	$(MAKE) -C lib/libtommath

build-libtomcrypt: build-libtommath
	$(MAKE) -C lib/libtomcrypt
...
build-libdom: build-libhubbub build-libwapcaplet
	$(MAKE) -C lib/libdom
...
test-libtommath: build-libtommath
	$(MAKE) -C tests/libtommath

test-libtomcrypt: build-libtomcrypt
	$(MAKE) -C tests/libtomcrypt
...
test-libnslog: build-libnslog
	$(MAKE) -C tests/libnslog run

test-libnsutils: build-libnsutils
