...
- `lib/libjpeg/` — JPEG image decoder + encoder (IJG libjpeg 9f, ~272 KB at -O0). Upstream `https://www.ijg.org/files/jpegsrc.v9f.tar.gz` (libjpeg 9f), IJG license. 44 hand-written .c files (37 LIBSOURCES from upstream Makefile.unix MINUS `jfdctflt.c` + `jidctflt.c` -- the FLOAT DCT/IDCT variants -- PLUS `jmemnobs.c` (no-backing-store memory manager)). Public headers: `jpeglib.h` + `jmorecfg.h` (with the same amiport patch as src/). **Critical config: `DCT_FLOAT_SUPPORTED` is gated behind `#ifdef JPEG_AMIPORT_FLOAT_DCT` in BOTH `src/jmorecfg.h` and `include/jmorecfg.h` (kept identical), and we never define that macro.** This excludes the float DCT path from compilation entirely. Why: float DCT uses `double` arithmetic that would either pull soft-float (on 68000 / non-FPU) or trigger the FS-UAE 68882 transcendental gap (crash-patterns #2 family). The integer DCT methods (`JDCT_ISLOW` -- the libjpeg default -- and `JDCT_IFAST`) provide equivalent functionality. Consumers must NOT request `JDCT_FLOAT` at runtime (will fire `JERR_NOT_COMPILED` and longjmp via `error_exit`). Verified post-build: zero soft-float symbols, zero libm transcendentals. Built `-O0 -fno-strict-aliasing -m68040 -m68881 -DNDEBUG -std=c99` whole-archive (per crash-patterns #16). Memory-checker APPROVED with no findings (pool-based allocator in `jmemmgr.c` plus `jmemnobs.c` malloc/free wrappers; `jpeg_destroy_decompress` walks all pools and frees everything; setjmp recovery is safe; `cinfo->mem != NULL` guard makes destroy idempotent). 18/18 tests pass on vamos `-C 68040 -s 1024 -m 4096`. **CRITICAL vamos resource sizing:** test binaries linking libjpeg need `__stack >= 524288` AND `__MEMORY_STEP >= 524288` (libpng-class). Test fixture is the upstream IJG `testimg.jpg` (227x149 RGB, 5770 bytes) embedded inline as `static const` array via `xxd -i` -- no filesystem touchpoints during test. **Mandatory consumer cleanup:** `jpeg_destroy_decompress` for every successfully-created struct, INCLUDING from setjmp recovery paths (the libjpeg pool manager is robust to half-built cinfo). Required by future `ports/netsurf/` for `<img src="X.jpg">` decoding. **Standalone -- no external deps**, despite being one of the largest libs in the dep stack. Top-level: `make build-libjpeg`, `make test-libjpeg`.
- `lib/libpng/` — PNG image decoder + encoder (~215 KB at -O0, FIRST EXTERNAL lib in NetSurf-Vampire dep stack). Upstream `glennrp/libpng` libpng16 @ commit `8c62c3b` (libpng 1.6.x), zlib license. 15 hand-written .c files (~33K LOC) + 4 internal headers + 2 public headers + generated `pnglibconf.h` (committed; derived from `scripts/pnglibconf.h.prebuilt` with one critical amiport patch). **Critical config: `PNG_FLOATING_ARITHMETIC_SUPPORTED` and `PNG_FLOATING_POINT_SUPPORTED` are commented out in `src/pnglibconf.h`.** This avoids `pow`/`floor`/`frexp` calls that would either pull soft-float helpers (on 68000 / non-FPU) or trigger the FS-UAE 68882 transcendental gap (crash-patterns #2 family). The fixed-point gamma path (`png_fixed_point` = `int32_t`, 1/100000 units) provides equivalent functionality via `png_set_gAMA_fixed`/`png_get_gAMA_fixed` and the other `*_fixed` API variants -- consumers must use these instead of the float APIs. Verified post-build: zero soft-float symbols, zero libm transcendental calls. Built `-O0 -fno-strict-aliasing -m68040 -m68881 -DNDEBUG -D_DEFAULT_SOURCE -DPNG_NO_CONSOLE_IO -std=c99` whole-archive (per crash-patterns #16 default-O0-for-bundled-libs); Stage 7 perf-optimizer audit will promote hot files to -O1 after struct-return audit. **Depends on lib/zlib** (link order: `-lpng -lz`). Memory-checker / perf-optimizer audits in flight; 18/18 tests pass on vamos `-C 68040 -s 1024 -m 4096`. **CRITICAL vamos resource sizing:** test binaries linking libpng + zlib need `__stack >= 524288` AND `__MEMORY_STEP >= 524288` (256 KB defaults that work for standalone NetSurf utility libs fail with no stdout output and exit 20). Test fixture (1x1 RGBA red pixel) is generated AT TEST INIT via libpng's own write side -- avoids fragile hand-crafted CRC/deflate stream. Mandatory consumer cleanup: `png_destroy_read_struct` / `png_destroy_write_struct` for every successfully-created struct, including from setjmp recovery paths. Required by future `ports/netsurf/` for `<img src="X.png">` decoding. Top-level: `make build-libpng`, `make test-libpng`.
- `lib/libnspsl/` — NetSurf Public Suffix List lookup (~67 KB; ~64 KB is `static const` PSL data, ~3 KB is code). Upstream `netsurf-browser/libnspsl` v0.1.7 @ commit `82815c2`, MIT-licensed. 1 hand-written .c (~208 LOC) + 1 generated `psl.inc` (~13K LOC of `static const` `union pnode[]` graph + Huffman string table). Single API: `const char *nspsl_getpublicsuffix(const char *hostname)` -- returns the **registrable domain (eTLD+1)**, NOT the public suffix itself (per the PSL algorithm: "domain must match public suffix plus one additional label"; e.g. `bbc.co.uk` -> `bbc.co.uk`, `www.example.com` -> `example.com`, `a.b.c.example.co.uk` -> `example.co.uk`). Returns a pointer **INTO** the caller's input buffer (no allocation; nothing for the caller to free). Built `-O1 -fno-strict-aliasing -m68040 -m68881 -DNDEBUG -D_DEFAULT_SOURCE -std=c99` whole-archive, audited 2026-05-02 -- single TU trivially -O1-safe (no struct returns, integer-only). Memory-checker APPROVED with no findings (zero allocations across the entire library; `static const` data, re-entrant). Source-analyzer CLEAN (only `<stdint.h>` + `<string.h>`, maximally freestanding). 18/18 tests pass on vamos `-C 68040 -s 1024 -m 4096` (standalone-lib class). **Despite the original Phase D-prime planning notes**, libnspsl does NOT depend on libwapcaplet -- the public API is just `const char *` in / `const char *` out. **Mandatory consumer cleanup:** NONE. PSL data table is regenerated from `public_suffix_list.dat` (vendored alongside) via `src/genpubsuffix.pl` (Perl) when upstream PSL data refreshes. Required by future `ports/netsurf/` for cookie domain scope checks. Top-level: `make build-libnspsl`, `make test-libnspsl`.
- `lib/libnsutils/` — NetSurf utility library (~3 KB). Upstream `netsurf-browser/libnsutils` v0.1.0+ @ commit `0bd3906`, MIT-licensed. 3 TUs: `src/time.c` (~100 LOC, monotonic ms via ReadEClock), `src/unistd.c` (~70 LOC, +1 amiport ftruncate-fallback removal, pwrite/pread emulation), `src/base64.c` (~430 LOC, RFC4648 base64 + URL-safe variants, 8 functions). 6 public headers: `time.h`, `unistd.h`, `base64.h`, `errors.h`, `endian.h` (inline-only), `assert.h` (macro-only). Built `-O1 -fno-strict-aliasing -m68040 -m68881 -DNDEBUG -D_DEFAULT_SOURCE -std=c99` whole-archive, audited 2026-05-02 -- all 3 TUs cleared crash-patterns #16 (no struct returns >8 bytes; nsuerror is 4 bytes; ssize_t is 4 bytes) and #2 (zero soft-float pulls verified via `m68k-amigaos-nm`; 64-bit divide via `___udivdi3` is INTEGER libgcc helper, not float). **Source-analyzer fix at `src/unistd.c`:** original upstream `nsu_pwrite` had an `if (errno == ESPIPE) { ftruncate(fd, offset); }` fallback for past-EOF seeks; libnix `-noixemul` does NOT provide ftruncate, so we removed the fallback (consumer must pre-grow files before pwrite-past-EOF; documented inline). Memory-checker APPROVED -- zero realloc, only 2 malloc sites (the `_alloc` base64 variants which transfer ownership to caller). **CRITICAL consumer requirement for `nsu_getmonotonic_ms`:** caller MUST open `timer.device` and provide `TimerBase` global before invoking; libnix does NOT auto-open timer.device. Test binary's `open_timer_device()` shows the canonical `CreateMsgPort` + `CreateIORequest` + `OpenDevice` pattern. 22/22 tests pass on vamos `-C 68040 -s 1024 -m 4096` (standalone-lib class). **Mandatory consumer cleanup:** caller frees every `_alloc` result from `nsu_base64_*_alloc()`. Required by future `ports/netsurf/` (monotonic time for render-tick scheduling, base64 for HTTP Basic Auth + data URIs, pwrite for cache file management). Top-level: `make build-libnsutils`, `make test-libnsutils`.
- `lib/libnslog/` — NetSurf logging library (~21 KB). Upstream `netsurf-browser/libnslog` v0.1.3 @ commit `bedff21`, MIT-licensed. 4 TUs: `src/core.c` (~202 LOC + 1 amiport vsnprintf-NULL fix), `src/filter.c` (~405 LOC), plus pre-generated flex `filter-lexer.inc/.h` (~3 KB) + bison `filter-parser.c/.h` (~1.8 KB) committed to the tree (build is host-tool-free). Provides 7 log levels (DEEPDEBUG/DEBUG/VERBOSE/INFO/WARNING/ERROR/CRITICAL) with compile-time + runtime threshold filtering, hierarchical categories declared via `NSLOG_DEFINE_CATEGORY`/`NSLOG_DEFINE_SUBCATEGORY` macros (lazy slash-separated path normalisation), one-shot startup "cork" that buffers messages until callback installation + `nslog_uncork()`, reference-counted filter trees buildable programmatically OR from textual filter language (e.g. `(level: WARNING) && (cat: netsurf/render)` -- **UPPERCASE level names mandatory**). Built `-O1 -fno-strict-aliasing -m68040 -m68881 -DNDEBUG -D_DEFAULT_SOURCE -std=c99` whole-archive, audited 2026-05-02 -- all 4 TUs cleared crash-patterns #16 (no struct returns >8 bytes; `nslog_error` enum is 4 bytes) and #2 (zero soft-float pulls verified via `m68k-amigaos-nm`). **Source-analyzer fix at `src/core.c:137`:** original upstream used the libnix-incompatible NULL-buffer probe pattern (crash-patterns #5); replaced with a 1024-byte stack probe buffer. Memory-checker APPROVED with no critical findings (all 8 alloc sites have matching free paths, refcount discipline at filter.c:218 sound, cork chain drained by `nslog_uncork`/`nslog_cleanup`). 25/25 tests pass on vamos `-C 68040 -s 1024 -m 4096` (standalone-lib class -- 256 KB cookies sufficient). **CRITICAL: cork is one-shot.** `nslog_uncork()` performs a process-wide one-time transition; `nslog_cleanup()` does NOT reset cork state. Test design accounts for this -- the `corked_buffers_messages_until_uncork` test runs FIRST before any other test calls uncork. **Mandatory consumer cleanup:** `nslog_cleanup()` before exit (frees ~500 bytes to 2 KB of category names + filter tree + cork residue). Required by future `ports/netsurf/`. **Standalone -- no NetSurf-internal deps**, despite being the first Wave 3 lib in Phase D-prime. Top-level: `make build-libnslog`, `make test-libnslog`. To regenerate flex/bison output (only when upstream `.l`/`.y` change): see `lib/libnslog/Makefile` header for the recipe (requires Homebrew bison 3.x on macOS -- system bison 2.3 is too old).
