		...
		...)
{
	...
	va_start(ap, pattern);
	if (nslog__corked) {
		/* amiport: libnix vsnprintf does NOT support NULL destination
		 * (crash-patterns #5). Use a fixed probe buffer to measure the
		 * required length. core.c is non-recursive; 1024 is stack-safe.
		 */
		char probe[1024];
		va_list ap2;
		va_copy(ap2, ap);
