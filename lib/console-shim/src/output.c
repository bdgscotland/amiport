...
/* Color pair storage */
static struct { short fg, bg; } color_pairs[COLOR_PAIRS];
...
static void emit_attrs(attr_t attrs)
{
    ...
    if (pair > 0 && pair < COLOR_PAIRS) {
        if (color_pairs[pair].fg >= 0)
            printf(";%d", 30 + color_pairs[pair].fg);
        if (color_pairs[pair].bg >= 0)
            printf(";%d", 40 + color_pairs[pair].bg);
    }
