...
# elif defined(__AMIGA__)
	/* amiport: AmigaOS 3.x has no /dev/urandom and no getentropy. The
	 * posix-shim also lacks getppid/getpgid/getsid (no process hierarchy
	 * on classic AmigaOS). Skip the random device probe entirely and
	 * fall directly through to time + stack-address entropy mixing. */
	GIT_UNUSED((fd = 0));
# else
	/*
...
#else
	/* amiport: no process-hierarchy IDs; use stack-address entropy in
	 * their place. Stack positions vary per invocation under normal
	 * AmigaOS task allocation -- not cryptographic, but sufficient for
	 * the hash-randomization use case libgit2 exercises here. */
	*seed ^= ((uint64_t)((size_t)((void *)&tv)) << 32);
	*seed ^= ((uint64_t)((size_t)((void *)&fd)) << 16);
#endif
