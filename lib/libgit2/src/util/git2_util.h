...
/* amiport: 64 KiB is unsafe as a local stack array on 68k with amiport's
 * standard __stack=262144 (real AmigaOS adds 2-4 KB hidden overhead per
 * dos.library call, and some libgit2 call chains recurse across tree and
 * pack walkers). Reduce to 4 KiB on AmigaOS. All four known FILEIO
 * consumers (filebuf, futils:cp_by_fd, blob content read, odb loose read)
 * are non-recursive bulk I/O loops where trip count scales with file size,
 * not buffer size, so the throughput impact is negligible on a 68k system
 * where filesystem and DMA overhead dominate. Source patched here (macro
 * level) rather than per-call-site so it covers every GIT_BUFSIZE_* user
 * uniformly and any future upstream additions pick it up automatically.
 * Reverting this on a platform with more stack is a one-line change. */
