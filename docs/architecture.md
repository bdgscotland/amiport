...
- **regression-checker**: Uses Sonnet to detect regressions after shim/library changes. Identifies ports linked against the changed library, rebuilds them, runs vamos smoke tests, and reports failures. Does not fix regressions — just reports them.
- **catalog-engineer**: Uses Sonnet for catalog management — candidate enumeration, dry-run analysis, readiness scoring, and batch dispatch coordination.
- **sdl-game-helper**: Uses Sonnet for SDL1/SDL2 game-port expertise. Dispatched for any new SDL/game port (after `aminet-researcher` confirms it isn't already on Aminet) or to audit an existing game port's performance. Flexible-style specialist with a 6-phase checklist: toolchain + build hygiene (gcc13 image, thread stubs, soft-float linkage, CMake flag discipline), libstdc++ ABI traps (bebbo-gcc 13.3 `std::ostream<<int` Guru, `std::string operator+` at -O0, `sleep_for` granularity), libSDL2-amigaos3 fast-path (`OS3_OpenWindowed` +64 padding, asm bswap32, `-O2` override), game-loop architecture (frame-budget math, blitter mode, OS cursor), FS-UAE configuration (CPU superset rule, FPU compatibility, uaegfx.card staging), and ReadEClock profiling discipline. Reference ports: `ports/openttd/` (PDR-015) and Vanilla Conquer at `~/Developer/libSDL2-amigaos3/ports/vanilla-conquer/`.
...
- **Cursor verification** (ADR-025): `SCREEN_READ` + `EXPECT_TRAP_CURSOR row,col` reads cursor position from ConUnit via custom FS-UAE trap mode 150. ScreenRead (`toolchain/screenread/`) triggers the trap; the host reads ConUnit fields (cu_XCCP+62, cu_YCCP+64) from emulated memory. Authoritative for COOKED mode interactive cursor operations. For RAW mode programs (mg, less, nano), ConUnit cursor stays at (0,0) -- use `EXPECT_AT` on the program's status line instead.
- **Host-side key injection** (ADR-025): Visual tests use `scripts/inject-keys.sh` (macOS osascript) instead of Amiga-side KeyInject. AddIEvents() does not reliably deliver to RAW mode programs. The ARexx harness coordinates via sentinel files (`keys-request-N` / `keys-done-N`). `CLEANUP:` directive sends quit keys after SCRAPE capture.
