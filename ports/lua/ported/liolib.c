...
/* amiport: include shim for amiport_tmpfile() */
#ifdef __AMIGA__
...
static int io_tmpfile (lua_State *L) {
  LStream *p = newfile(L);
  errno = 0;
  /* amiport: replaced tmpfile() with amiport_tmpfile() — uses T: on AmigaOS */
#ifdef __AMIGA__
  p->f = amiport_tmpfile();
#else
  p->f = tmpfile();
...
static int read_line (lua_State *L, FILE *f, int chop) {
  luaL_Buffer b;
  int c = EOF;  /* amiport: init to EOF so loop exit is defined if fgets reads 0 */
  luaL_buffinit(L, &b);
