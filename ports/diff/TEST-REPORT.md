...
|-------|-------|
| Port | diff |
| Date | 2026-03-21 15:28:34 |
| Duration | 90s |
| Platform | FS-UAE 3.2.35 (A1200, Kickstart 3.1) |
| Binary | `WORK:diff` (55K) |
...
```
TEST: Identical files produce no output (exit 0)
CMD: WORK:diff WORK:test-diff-same.txt WORK:test-diff-same.txt
EXPECT:

TEST: Different files produce normal diff output
CMD: WORK:diff WORK:test-diff-a.txt WORK:test-diff-b.txt
EXPECT: grep 1c1

TEST: Unified diff format works
