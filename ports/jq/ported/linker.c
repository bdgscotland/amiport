...
#include <stdlib.h>
#include <stdint.h>
/* amiport: replaced <sys/stat.h> with amiport shim — provides stat() macro */
#include <amiport/sys/stat.h>
/* amiport: removed <libgen.h> — not in cross-compiler sysroot.
 * dirname() is in libnix libc.a; declare directly. */
...
static int path_is_relative(jv p) {
  ...
  /* amiport: AmigaOS absolute paths contain ':' (e.g. "SYS:foo", "Work:bar/baz").
   * Paths starting with '/' are also treated as absolute (device-relative).
   * A path is relative if it has no ':' and does not start with '/'. */
  int res = (*s != '/') && (strchr(s, ':') == NULL);
#else
  int res = *s != '/';
#endif
  jv_free(p);
...
static int process_dependencies(jq_state *jq, jv jq_origin, jv lib_origin, block *src_block, struct lib_loading_state *lib_state) {
  ...
  block bk = *src_block;
  int nerrors = 0;
  int ndeps = jv_array_length(jv_copy(deps));
  ...
  for (int i = ndeps; i > 0; ) {
    ...
    if (jv_get_kind(jv_object_get(jv_copy(dep), jv_string("optional"))) == JV_KIND_TRUE)
      optional = 1;
    jv_free(v);
    jv relpath_raw = jv_object_get(jv_copy(dep), jv_string("relpath"));
    jv relpath = validate_relpath(relpath_raw);
...
int load_program(jq_state *jq, struct locfile* src, block *out_block) {
...

  {
    ...
    jv prog = jq_get_prog_origin(jq);
    nerrors = process_dependencies(jq, origin, prog, &program, &lib_state);
  }
  block libs = gen_noop();
