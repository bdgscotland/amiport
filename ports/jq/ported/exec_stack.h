...
/* amiport: force minimum 4-byte alignment. On 68k, offsetof gives 2 (68000 only
   requires word alignment), but the stack stores stack_ptr (int, 4 bytes) before
   each block via stack_block_next. With 2-byte alignment, these ints can land at
   non-4-byte-aligned addresses, causing misaligned reads that corrupt data. */
enum {ALIGNMENT = (offsetof(struct determine_alignment, u) < 4) ? 4 : offsetof(struct determine_alignment, u)};
