...
#include <ctype.h>
#include <errno.h>
/* amiport: removed <libgen.h> — not in cross-compiler sysroot.
 * dirname() is in libnix libc.a; declare directly below. */
...
/* amiport: libnix does not export the POSIX environ array. AmigaOS environment
 * variables live in ENV: and are accessed via GetVar()/SetVar(). Provide an
 * empty environ so jq's $ENV builtin compiles and returns an empty object. */
char *_amiport_empty_environ[] = { NULL };
char **environ = _amiport_empty_environ;
...
#include "jv_alloc.h"
#include "util.h"
/* amiport: fixed include paths — files are in ported/ directory */
#include "version.h"
#include "config_opts.inc"

/* amiport: jq_testsuite is in jq_test.c which requires pthreads — not available
 * on AmigaOS. Stub returns JQ_ERROR_SYSTEM so --run-tests prints an error message. */
int jq_testsuite(jv lib_dirs, int verbose, int argc, char* argv[]) {
  (void)lib_dirs; (void)verbose; (void)argc; (void)argv;
  fprintf(stderr, "jq: --run-tests not supported on AmigaOS (requires pthreads)\n");
  return 5;
}
...
static void usage(int code, int keep_it_short) {
  ...
  /* amiport: remap exit codes — code=2 (write error) becomes RETURN_FAIL (20) */
  {
    int ec = (ret < 0 && code == 0) ? 2 : code;
    if (ec == 2) ec = 20;
    exit(ec);
  }
}
...
/* amiport: remap jq exit codes to Amiga conventions.
 * JQ_OK_NULL_KIND(-1) and JQ_OK_NO_OUTPUT(-4) → exit(0) (no match, not error)
 * JQ_ERROR_SYSTEM(2), JQ_ERROR_COMPILE(3) → exit(20) RETURN_FAIL
 * JQ_ERROR_UNKNOWN(5) → exit(5) RETURN_WARN (non-fatal)
 * abs(r) for user-provided halt_error codes: remap 1→10, 2/3→20 */
static int jq_amiga_exit_code(int r) {
    int a = abs(r);
    if (a == 0) return 0;
    if (a == 1) return 10; /* RETURN_ERROR */
    if (a == 2 || a == 3) return 20; /* RETURN_FAIL */
    return a; /* 5=WARN, 10=ERROR, 20=FAIL pass through */
}
#define jq_exit_with_status(r)  exit(jq_amiga_exit_code(r))
#define jq_exit(r)              exit( (r) > 0 ? jq_amiga_exit_code(r) : 0 )
