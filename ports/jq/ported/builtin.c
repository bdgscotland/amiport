...
#endif
/* amiport: suppress strptime() and timeval macros from shim — we define
 * our own strptime wrapper below that bridges struct tm to struct amiport_tm.
 * The two structs have identical field layout so the cast is safe. */
#define AMIPORT_NO_TIME_MACROS 1
/* amiport: replaced <sys/time.h> with amiport shim — provides gettimeofday() */
#include <amiport/sys/time.h>
/* amiport: alias POSIX names to amiport shim types/functions — shim lacks these macros */
#define timeval amiport_timeval
#define gettimeofday(tv, tz) amiport_gettimeofday(tv)
#include <stdlib.h>
#include <stddef.h>
#ifdef HAVE_ALLOCA_H
# include <alloca.h>
...
#ifdef __AMIGA__
/* amiport: strptime bridge — struct tm and struct amiport_tm have identical layout
 * (same fields, same order, same sizes). Cast is safe on 68k AmigaOS.
 * time.h declares strptime() but libnix does not provide it — we define it here. */
char *strptime(const char *s, const char *fmt, struct tm *tm) {
  return amiport_strptime(s, fmt, (struct amiport_tm *)(void *)tm);
}
#endif
...
/* amiport: fixed include path — file is in ported/ not src/ */
#include "builtin.inc"
