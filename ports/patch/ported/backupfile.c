...
#include <ctype.h>
/* amiport: dirent provided via amiport/dirent.h (also provides mkdir macro) */
...
/* amiport: renamed concat -> backup_concat to avoid conflict with libnix concat() */
static char	*backup_concat(const char *, const char *);
...
find_backup_file_name(const char *file)
{
	...
	if (backup_type == simple)
		return backup_concat(file, simple_backup_suffix);
	tmp_file = strdup(file);
	if (tmp_file == NULL)
		return NULL;
	base_versions = backup_concat(basename(tmp_file), ".~");
	free(tmp_file);
...
max_backup_version(const char *file, const char *dir)
{
	...
	while ((dp = readdir(dirp)) != NULL) {
		/* amiport: replaced d_namlen with strlen(dp->d_name) — AmigaOS
		 * dirent does not populate d_namlen (always 0) */
 ...
 * memory, return 0.
 * amiport: renamed from concat() to backup_concat() — libnix defines a
 * conflicting concat(const char *, ...) with __stdargs calling convention.
 */
static char  *
backup_concat(const char *str1, const char *str2)
{
	char	*newstr;
...
get_version(const char *version)
{
	...
	exit(10); /* amiport: RETURN_ERROR — exit(2) invisible to AmigaOS shell */
}
