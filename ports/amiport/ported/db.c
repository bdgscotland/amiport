 ...
 *
 * Read-modify-write S:amiport.db with temp file + AmigaDOS Rename.
 *
...
#define DB_PATH "S:amiport.db"
#define DB_TMP  "S:amiport.db.tmp"

/* Static storage for find results */
static struct amiport_installed db_find_result;

/* Static storage for full DB */
static struct amiport_installed db_entries[AMIPORT_MAX_INSTALLED];
...
static int db_reload(void)
{
    ...
    while (fgets(line, sizeof(line), fp) && db_count < AMIPORT_MAX_INSTALLED) {
        char *name_end;
        char *ver_end;
        char *path_start;
        int len;
...
int amiport_db_save(const char *name, const char *version, const char *path)
{
    ...
    /* New entry */
    if (db_count >= AMIPORT_MAX_INSTALLED) return -1;
        ...
        if (strcmp(db_entries[i].name, name) == 0) {
            ...
            if (i < db_count - 1) {
                memmove(&db_entries[i], &db_entries[i + 1],
                        (db_count - i - 1) * sizeof(struct amiport_installed));
            }
            db_count--;
            found = 1;
            break;
