...
ok 12 - upgrade amiport blocked by self-update guard returns RC 10
...
| 12 | upgrade amiport blocked by self-update guard returns RC 10 | PASS | |
...
```
# test-fsemu-cases.txt -- amiport 1.0 test suite
#
...
# Native reference: amiport (no args) -- prints same as help
TEST: No arguments prints help text
CMD: WORK:amiport
EXPECT: amiport - Package manager for AmigaOS 3.x
EXPECT_RC: 0

# Native reference: amiport help
TEST: help command prints usage header
CMD: WORK:amiport help
EXPECT: amiport - Package manager for AmigaOS 3.x
EXPECT_LINE: 3,Usage: amiport <command> [arguments]
EXPECT_RC: 0
...
CMD: WORK:amiport search xyzzy-no-such-package-ever
...
CMD: WORK:amiport info xyzzy-no-such-package-ever
EXPECT_RC: 10

# -----------------------------------------------------------------------
# Group C: Install lifecycle (stateful -- tests must run in order)
# These tests share the S:amiport.db filesystem state across the session.
# -----------------------------------------------------------------------
...
# hits S:amiport-cache.json, not the network)
...
---
Generated by `make test-fsemu TARGET=ports/amiport`
