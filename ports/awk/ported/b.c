...
Node *awk_concat(Node *np)	/* amiport: renamed from concat -- clashes with libnix string.h */
{
	switch (rtok) {
	case CHAR: case DOT: case ALL: case CCL: case NCCL: case '$': case '(':
		return (awk_concat(op2(CAT, np, primary())));
	case EMPTYRE:
		rtok = relex();
		return (awk_concat(op2(CAT, op2(CCL, NIL, (Node *) cclenter("")),
				primary())));
	}
	return (np);
}
