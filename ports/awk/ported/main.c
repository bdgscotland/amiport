...
/* amiport: stack cookie and version string for AmigaOS */
long __stack = 65536;
static const char *verstag __attribute__((used)) = "$VER: awk 2024.12.25 (25.03.2026)";

#define DEBUG
#include <stdio.h>
#include <ctype.h>
#include <locale.h>
#include <amiport/stdlib.h>	/* amiport: exit() -> amiport_exit() macro */
#include <string.h>
#include <signal.h>
#include "awk.h"

/* amiport: libnix does not provide environ -- define an empty array so
 * envinit() iterates nothing and ENVIRON is an empty AWK array. */
#ifdef __AMIGA__
static char *_amiport_empty_environ[] = { NULL };
...
/* amiport: AmigaOS toolchain defines SA_SIGINFO but lacks sa_sigaction and
 * siginfo_t in sigaction struct -- disable SA_SIGINFO path entirely on Amiga */
#ifdef __AMIGA__
#undef SA_SIGINFO
#endif
...
/* amiport: force awk_mb_cur_max=1 on AmigaOS -- libnix MB_CUR_MAX is a
 * function call that may return >1 even without locale support (crash-patterns #11).
 * awk's multibyte paths are not useful on AmigaOS and cause wrong output. */
#ifdef __AMIGA__
	awk_mb_cur_max = 1;
#else
	...
	if (argc == 1) {
		fprintf(stderr,
		  ...
		  cmdname);
		exit(10);	/* amiport: exit(1) -> exit(10) RETURN_ERROR for AmigaOS shell scripts */
	}
#ifdef SA_SIGINFO
	{
		struct sigaction sa;
	...
	srand_seed = 1;
	srand((unsigned long) srand_seed);	/* amiport: random() missing from libnix -- use srand/rand */
