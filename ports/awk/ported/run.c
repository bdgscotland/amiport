...
#include <time.h>
#include <sys/types.h>
/* amiport: fcntl.h above already pulls in sys/stat.h, causing typedef conflicts
 * with amiport/sys/stat.h. Suppress the amiport typedef/macro aliases and define
...
/* amiport: removed <sys/wait.h> -- not available on AmigaOS */
/* amiport: libnix stdio.h does not define FOPEN_MAX; use ANSI C minimum of 20 */
#ifndef FOPEN_MAX
#define FOPEN_MAX 20
#endif
#include <amiport-emu/popen.h>	/* amiport-emu: popen/pclose/system via temp files */

#ifdef __AMIGA__
/* amiport: W* macros -- AmigaOS system() returns exit code directly */
#define WIFEXITED(s)    1
#define WEXITSTATUS(s)  (s)
#define WIFSIGNALED(s)  0
#define WTERMSIG(s)     0
#define WCOREDUMP(s)    0
/* amiport: fcntl() not available on AmigaOS -- stub as no-op */
...
/* amiport: AmigaOS toolchain wctype.h provides towupper/towlower -- no stubs needed */
#endif
...
Cell *bltin(Node **a, int n)	/* builtin functions. a[0] is type, a[1] is arg list */
{
	...
	case FSYSTEM:
		...
		break;
	case FRAND:
		/* amiport: random() missing from libnix -- use rand() which returns [0..RAND_MAX]
		 * RAND_MAX on AmigaOS libnix is 2^31-1, matching the original range.
		 * in order to get a number in [0, 1), divide by 2^31
		 */
		do {
			/* exact if Awkfloat wide enough */
			u = (Awkfloat) rand();	/* amiport: random() -> rand() */
	...
	case FSRAND:
		if (isrec(x))	/* no argument provided */
			...
			u = getfval(x);
		tmp = u;
		srand((unsigned long) u);	/* amiport: srandom() -> srand() */
		u = srand_seed;
		srand_seed = tmp;
		break;
	case FTOUPPER:
...
FILE *openfile(int a, const char *us, bool *pnewflag)
{
	...
	if (i >= nfiles) {
		...
		if (nf == NULL)
			FATAL("cannot grow files for %s and %lu files", s, (unsigned long)nnf);	/* amiport: %zu -> %lu, libnix lacks C99 size_t format */
	...
	stat = true;
 	for (i = 0; i < nfiles; i++) {
		...
		if (files[i].fp == stdin || files[i].fp == stdout ||
		    files[i].fp == stderr)
			stat = freopen("NIL:", "r+", files[i].fp) == NULL;	/* amiport: /dev/null -> NIL: (AmigaOS null device) */
