...
_Py_GetLocaleEncoding(void)
{
    ...
    /* amiport: AmigaOS has no locale system. Always return ASCII. */
    return _PyMem_RawWcsdup(L"ascii");
#else
...
{
#ifdef __AMIGA__
    /* amiport: AmigaOS has no process inheritance -- fcntl not in libnix.
       Report all fds as inheritable (1) to satisfy callers that check before
       calling set_inheritable. set_inheritable always succeeds below. */
    return 1;
...
{
#ifdef __AMIGA__
    /* amiport: AmigaOS has no process inheritance or close-on-exec semantics.
       fcntl() is not in libnix. Always succeed silently. */
    (void)fd; (void)inheritable; (void)raise; (void)atomic_flag_works;
    return 0;
#else  /* !__AMIGA__ */
#ifdef MS_WINDOWS
    HANDLE handle;
...
_Py_isabs(const wchar_t *path)
{
    ...
    return 1;
#elif defined(__AMIGA__)
    /* amiport: AmigaOS paths containing ':' are absolute (device-relative).
     * WORK:file.py, SYS:libs/foo, PROGDIR:data are all absolute.
     * Plain foo.py or subdir/foo.py are relative. */
...
{
#ifdef __AMIGA__
    /* amiport: AmigaOS has no non-blocking I/O -- fcntl not in libnix.
       All fds are always blocking. */
    return 1;
#else
    int flags;
    _Py_BEGIN_SUPPRESS_IPH
...
#ifdef __AMIGA__
    /* amiport: AmigaOS has no non-blocking I/O -- fcntl not in libnix.
       Silently ignore; all I/O is always blocking. */
    return 0;
#endif
