...
{
#ifdef __AMIGA__
    /* amiport: Disable bytecode quickening on 68k big-endian.
     * _Py_SET_OPCODE writes the opcode byte at an offset within the
     * 16-bit _Py_CODEUNIT that is wrong on big-endian, corrupting
     * the bytecode and causing ACPU_InstErr (Guru Meditation #80000004)
     * after 128 executions of a code object. The specializing adaptive
     * interpreter (PEP 659) is a performance optimization that we
     * sacrifice for correctness on 68k. */
    (void)code;
    return;
#endif
    _Py_QuickenedCount++;
