...
PyImport_ImportFrozenModuleObject(PyObject *name)
{
    ...
    fprintf(stderr, "F2: unmarshal\n"); fflush(stderr);
    co = unmarshal_frozen_code(&info);
    if (co == NULL) {
        fprintf(stderr, "F2: FAIL\n"); fflush(stderr);
        return -1;
    }
    fprintf(stderr, "F3: co=%p\n", (void*)co); fflush(stderr);
    if (info.is_package) {
    ...
    fprintf(stderr, "F4: dict\n"); fflush(stderr);
    d = module_dict_for_exec(tstate, name);
    if (d == NULL) {
        goto err_return;
    }
    fprintf(stderr, "F5: exec\n"); fflush(stderr);
    m = exec_code_in_module(tstate, name, d, co);
    fprintf(stderr, "F6: exec done m=%p\n", (void*)m); fflush(stderr);
    if (m == NULL) {
        goto err_return;
    }
    Py_DECREF(m);
