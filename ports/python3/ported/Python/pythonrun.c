...
{
    PyObject *m, *d, *v;
    fprintf(stderr, "CHK: SimpleString enter\n"); fflush(stderr);
    m = PyImport_AddModule("__main__");
    if (m == NULL)
        return -1;
    d = PyModule_GetDict(m);
    fprintf(stderr, "CHK: calling RunString\n"); fflush(stderr);
    v = PyRun_StringFlags(command, Py_file_input, d, d, flags);
    fprintf(stderr, "CHK: RunString returned\n"); fflush(stderr);
    if (v == NULL) {
        PyErr_Print();
        return -1;
    }
            ...
            PyCompilerFlags *flags, PyArena *arena)
{
    PyThreadState *tstate = _PyThreadState_GET();
    PyCodeObject *co;
    fprintf(stderr, "CHK: compile start\n"); fflush(stderr);
    co = _PyAST_Compile(mod, filename, flags, -1, arena);
    if (co == NULL)
        return NULL;
    fprintf(stderr, "CHK: compile done, eval start\n"); fflush(stderr);
