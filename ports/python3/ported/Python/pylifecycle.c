...
init_importlib(PyThreadState *tstate, PyObject *sysmod)
{
    ...
    fprintf(stderr, "I1: frozen importlib\n"); fflush(stderr);
    // Import _importlib through its frozen version, _frozen_importlib.
    if (verbose) {
        PySys_FormatStderr("import _frozen_importlib # frozen\n");
    }
    if (PyImport_ImportFrozenModule("_frozen_importlib") <= 0) {
        fprintf(stderr, "I1: FAILED\n"); fflush(stderr);
        return -1;
    }
    fprintf(stderr, "I2: add module\n"); fflush(stderr);
    ...
    fprintf(stderr, "I3: bootstrap imp\n"); fflush(stderr);
    ...
    fprintf(stderr, "I4: install\n"); fflush(stderr);
    // Install importlib as the implementation of import
    PyObject *value = PyObject_CallMethod(importlib, "_install",
                                          "OO", sysmod, imp_mod);
    Py_DECREF(imp_mod);
    if (value == NULL) {
        fprintf(stderr, "I4: FAILED\n"); fflush(stderr);
        return -1;
    }
    Py_DECREF(value);
    fprintf(stderr, "I5: importlib done\n"); fflush(stderr);
...
pycore_interp_init(PyThreadState *tstate)
{
    ...
    /* amiport: checkpoint tracing for crash diagnosis */
    fprintf(stderr, "P1: deepfreeze\n"); fflush(stderr);
    ...
    fprintf(stderr, "P2: types\n"); fflush(stderr);
    status = pycore_init_types(interp);
    if (_PyStatus_EXCEPTION(status)) {
        goto done;
    }

    fprintf(stderr, "P3: warnings\n"); fflush(stderr);
    if (_PyWarnings_InitState(interp) < 0) {
        return _PyStatus_ERR("can't initialize warnings");
    }

    fprintf(stderr, "P4: atexit\n"); fflush(stderr);
    status = _PyAtExit_Init(interp);
    if (_PyStatus_EXCEPTION(status)) {
        return status;
    }

    fprintf(stderr, "P5: sys\n"); fflush(stderr);
    status = _PySys_Create(tstate, &sysmod);
    if (_PyStatus_EXCEPTION(status)) {
        goto done;
    }

    fprintf(stderr, "P6: builtins\n"); fflush(stderr);
    status = pycore_init_builtins(tstate);
    if (_PyStatus_EXCEPTION(status)) {
        goto done;
    }

    fprintf(stderr, "P7: importlib\n"); fflush(stderr);
    ...
    fprintf(stderr, "P8: core init done\n"); fflush(stderr);
...
#endif
    /* amiport: on AmigaOS, skip _PyUnicode_InitEncodings -- it tries to
     * import the encodings module (.py files) which aren't on disk yet.
     * stdout/stderr will use ASCII codec (already registered as builtin). */
