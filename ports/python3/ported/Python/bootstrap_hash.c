...
pyurandom(void *buffer, Py_ssize_t size, int blocking, int raise)
{
    ...
    /* amiport: AmigaOS has no /dev/urandom. Use DateStamp() + task address
     * as entropy source. Not cryptographically secure but sufficient for
     * Python's hash randomization. Uses a simple LCG seeded from system
     * state to fill the buffer with pseudo-random bytes. */
    {
        unsigned char *buf = (unsigned char *)buffer;
        Py_ssize_t i;
        /* Seed from DateStamp ticks + stack pointer (ASLR-like) */
        unsigned long seed = (unsigned long)&i;  /* stack address */
