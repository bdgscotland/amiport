...
|-------|-------|
| Port | amigit |
| Date | 2026-04-15 21:17:03 |
| Duration | 17s |
| Platform | FS-UAE 3.2.35 (A1200, Kickstart 3.1) |
| Binary | `WORK:amigit` (1.2M) |
| Test method | ARexx harness → TAP output |
| Result | **PASS** — 103/103 passed |
...
ok 94 - clone missing url exits RETURN_ERROR
ok 95 - clone --help prints usage
ok 96 - clone rejects ftp:// URL scheme
ok 97 - clone rejects http:// URL (https-only)
ok 98 - clone rejects bare hostname without scheme
ok 99 - clone rejects https:// with empty host
ok 100 - clone rejects git:// URL scheme
ok 101 - clone rejects ssh:// URL scheme
ok 102 - clone too many positional args
ok 103 - clone unknown option
# passed: 103 failed: 0 total: 103
```
...
| 94 | clone missing url exits RETURN_ERROR | PASS | |
| 95 | clone --help prints usage | PASS | |
| 96 | clone rejects ftp:// URL scheme | PASS | |
| 97 | clone rejects http:// URL (https-only) | PASS | |
| 98 | clone rejects bare hostname without scheme | PASS | |
| 99 | clone rejects https:// with empty host | PASS | |
| 100 | clone rejects git:// URL scheme | PASS | |
| 101 | clone rejects ssh:// URL scheme | PASS | |
| 102 | clone too many positional args | PASS | |
| 103 | clone unknown option | PASS | |
...
```
# test-fsemu-cases.txt -- FS-UAE functional tests for amigit 0.2
#
...
EXPECT: amigit 0.2 (built 2026-04-15)
EXPECT_RC: 0
...
# ============================================================
# clone command -- PDR-012 Phase 8
# ============================================================
#
# `amigit clone <url> [path]` -- first user-facing networked command.
# Error-path tests run on FS-UAE without network; the happy-path
# clone of a real public repo is a real-hardware test (A2000 +
# Vampire + X-Surf 100 + Roadshow + AmiSSL) because FS-UAE
# bsdsocket passthrough + AmiSSL is not reliably available.

TEST: clone missing url exits RETURN_ERROR
CMD: WORK:amigit clone
EXPECT_CONTAINS: clone: missing url argument
EXPECT_RC: 10

TEST: clone --help prints usage
CMD: WORK:amigit clone --help
EXPECT_CONTAINS: usage: amigit clone
EXPECT_RC: 0

TEST: clone rejects ftp:// URL scheme
CMD: WORK:amigit clone ftp://example.com/repo.git
EXPECT_CONTAINS: unsupported URL scheme
EXPECT_RC: 10

TEST: clone rejects http:// URL (https-only)
CMD: WORK:amigit clone http://example.com/repo.git
EXPECT_CONTAINS: not supported
EXPECT_RC: 10

TEST: clone rejects bare hostname without scheme
CMD: WORK:amigit clone github.com/foo/bar.git
EXPECT_CONTAINS: not a URL
EXPECT_RC: 10

TEST: clone rejects https:// with empty host
CMD: WORK:amigit clone https:///repo.git
EXPECT_CONTAINS: no host
EXPECT_RC: 10

TEST: clone rejects git:// URL scheme
CMD: WORK:amigit clone git://example.com/repo.git
EXPECT_CONTAINS: unsupported URL scheme
EXPECT_RC: 10

TEST: clone rejects ssh:// URL scheme
CMD: WORK:amigit clone ssh://example.com/repo.git
EXPECT_CONTAINS: unsupported URL scheme
EXPECT_RC: 10

TEST: clone too many positional args
CMD: WORK:amigit clone https://example.com/r.git T:dest extra
EXPECT_CONTAINS: too many positional arguments
EXPECT_RC: 10

TEST: clone unknown option
CMD: WORK:amigit clone --depth 1 https://example.com/r.git
EXPECT_CONTAINS: unknown option
EXPECT_RC: 10
```
