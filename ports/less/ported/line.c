...
/* amiport: lookup tables for is_ansi_end/is_ansi_middle — replaces strchr()
 * over 24-char strings on every character in ANSI sequences. See perf-optimizer. */
static unsigned char ansi_end_table[256];
static unsigned char ansi_mid_table[256];
...
public void init_line(void)
{
	...
	/* amiport: build lookup tables for O(1) ANSI char classification */
	{
		constant char *p;
		memset(ansi_end_table, 0, sizeof(ansi_end_table));
		memset(ansi_mid_table, 0, sizeof(ansi_mid_table));
		for (p = end_ansi_chars; *p; p++)
			ansi_end_table[(unsigned char)*p] = 1;
		for (p = mid_ansi_chars; *p; p++)
			ansi_mid_table[(unsigned char)*p] = 1;
	}
...
{
	/* amiport: static pool replaces ecalloc — eliminates AllocMem/FreeMem
	 * on every ESC character. Safe because ansi_done() is always called
	 * before the next ansi_start(). See perf-optimizer findings. */
...
{
	/* amiport: no-op — ansi_start uses static pool, no free needed */
	(void) pansi;
}
...
static int store_ansi(LWCHAR ch, constant char *rep, POSITION pos)
{
	...
	case ANSI_END:
		...
		xbuf_reset(&last_ansi);
		/* amiport: avoid 68000 DIVU (120-158 cycles) for non-power-of-2 modulo */
		if (++curr_last_ansi >= NUM_LAST_ANSIS)
			curr_last_ansi = 0;
		break;
	case ANSI_ERR:
		{
