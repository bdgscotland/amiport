...
#include "bcdefs.h"
#include <amiport/signal.h>     /* amiport: replaced <signal.h>, provides amiport_check_break() */
#include <stdlib.h>             /* for rand() */
#include "proto.h"

/* amiport: libnix does not provide random() — map to rand() which is equivalent
   for bc's use (seeding non-cryptographic random numbers via the 'random' bc fn) */
#ifndef random
#define random() rand()
#endif
...
execute (void)
{
  ...
  while (pc.pc_addr < functions[pc.pc_func].f_code_size
    ...
    {
      /* amiport: poll for Ctrl-C — AmigaOS has no async SIGINT delivery */
