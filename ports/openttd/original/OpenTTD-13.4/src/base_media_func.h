...
bool BaseSet<T, Tnum_files, Tsearch_in_tars>::FillSetDetails(IniFile *ini, const char *path, const char *full_filename, bool allow_empty_filename)
{
	...
	diaglog("FSD: 2 after GetGroup metadata");
	IniItem *item;

	diaglog("FSD: 3 before fetch name");
	fetch_metadata("name");
	this->name = *item->value;
	diaglog("FSD: 4 after this->name");

	diaglog("FSD: 5 before fetch description");
	item = metadata->GetItem("description", false);
	diaglog("FSD: 5a after GetItem description");
	if (item == nullptr) { diaglog("FSD: 5b item==nullptr"); return false; }
	diaglog("FSD: 5c item not null");
	if (!item->value.has_value()) { diaglog("FSD: 5d no value"); return false; }
	...
	diaglog("FSD: 9 before fetch version");
	...
	diaglog("FSD: 10 before files group");
	...
	for (uint i = 0; i < Tnum_files; i++) {
		{ char b[60]; seprintf(b, lastof(b), "FSD: 11 file loop iter i=%u", i); diaglog(b); }
		...
		diaglog("FSD: 12 before CheckMD5");
		file->check_result = T::CheckMD5(file, BASESET_DIR);
		diaglog("FSD: 13 after CheckMD5");
...
{
	bool ret = false;
	/* AMIPORT: skip Debug() — fmt::format with std::string crashes on bebbo-gcc 13.3 */ /* Debug(grf, 1, "Checking {} for base " SET_TYPE " set", filename); */

	Tbase_set *set = new Tbase_set();
	diaglog("AF: 2 after new Tbase_set");
	IniFile *ini = new IniFile();
	diaglog("AF: 3 after new IniFile");
	{ char b[200]; seprintf(b, lastof(b), "AF: filename=%s basepath_length=%u", filename.c_str(), (unsigned)basepath_length); diaglog(b); }
	std::string path{ filename, basepath_length };
	{ char b[200]; seprintf(b, lastof(b), "AF: path.size=%u path.c_str=%s", (unsigned)path.size(), path.c_str()); diaglog(b); }
	diaglog("AF: 4 before LoadFromDisk");
	ini->LoadFromDisk(path, BASESET_DIR);
	diaglog("AF: 5 after LoadFromDisk");
	...
	diaglog("AF: 6 before FillSetDetails");
