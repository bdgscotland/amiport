...
bool VideoDriver_SDL_Base::ClaimMousePointer()
{
	...
	#ifdef __AMIGA__
	SDL_ShowCursor(1); /* PERF FIX: use OS hardware cursor instead of in-framebuffer software cursor.
	                    * Software cursor only redraws at game frame rate (~24 fps), feels laggy.
	                    * OS cursor draws at OS speed regardless of frame rate. PDR-015. */
#else
	SDL_ShowCursor(0);
#endif
#endif
...
{
	AMIPORT_PROFILE_BEGIN("SDL_LoopOnce");

	/* Periodic profile dump from the DRAW path -- the title menu does not
	 * call GameLoop() (no active game), so dumping only from there means
	 * profile.log never gets written. LoopOnce IS called every frame
	 * regardless of game state, so dump from here every 100 frames. */
#ifdef AMIPORT_PROFILE
	{
		static unsigned long sdl_loop_count = 0;
		sdl_loop_count++;
		if ((sdl_loop_count % 100) == 0) {
...
#endif
		AMIPORT_PROFILE_END("SDL_LoopOnce");
		return;
	}
