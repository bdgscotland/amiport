...
void IniLoadFile::LoadFromDisk(const std::string &filename, Subdirectory subdir)
{
	...
	size_t end;
	diaglog("LFD: 2 before OpenFile");
	FILE *in = this->OpenFile(filename, subdir, &end);
	{ char b[100]; seprintf(b, lastof(b), "LFD: 3 OpenFile returned %s end=%u", in ? "OK" : "NULL", (unsigned)end); diaglog(b); }
	if (in == nullptr) return;
	diaglog("LFD: 4 in not null");

	end += ftell(in);
	{ char b[100]; seprintf(b, lastof(b), "LFD: 5 after ftell, end=%u", (unsigned)end); diaglog(b); }

	/* for each line in the file */
	diaglog("LFD: 6 entering fgets loop");
