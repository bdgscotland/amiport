...
std::string FioGetDirectory(Searchpath sp, Subdirectory subdir)
{
	...
	/* AMIPORT: decomposed to avoid bebbo-gcc 13.3 -O0 std::string operator+ codegen bug */
	std::string r(_searchpaths[sp]);
	r += _subdirs[subdir];
	return r;
}
...
{
	diaglog("DSWD: 1 enter");
	/* No working directory, so nothing to do. */
	if (_searchpaths[SP_WORKING_DIR].empty()) return false;
	diaglog("DSWD: 2 not empty");

	/* Working directory is root, so do nothing. */
	if (_searchpaths[SP_WORKING_DIR] == PATHSEP) return false;
	diaglog("DSWD: 3 not root");

	/* No personal/home directory, so the working directory won't be that. */
	if (_searchpaths[SP_PERSONAL_DIR].empty()) return true;
	diaglog("DSWD: 4 personal not empty");
	{ char b[200]; seprintf(b, lastof(b), "DSWD: WORK_DIR.size=%u PERSONAL_DIR.size=%u", (unsigned int)_searchpaths[SP_WORKING_DIR].size(), (unsigned int)_searchpaths[SP_PERSONAL_DIR].size()); diaglog(b); }
	diaglog("DSWD: 4a calling c_str on WORK_DIR");
	{ char b[200]; seprintf(b, lastof(b), "DSWD: WORK_DIR=%s", _searchpaths[SP_WORKING_DIR].c_str()); diaglog(b); }
	diaglog("DSWD: 4b calling c_str on PERSONAL_DIR");
	{ char b[200]; seprintf(b, lastof(b), "DSWD: PERSONAL_DIR=%s", _searchpaths[SP_PERSONAL_DIR].c_str()); diaglog(b); }
	diaglog("DSWD: 4c about to do step 1: copy");
	std::string tmp(_searchpaths[SP_WORKING_DIR]);
	diaglog("DSWD: 4d after copy");
	{ char b[200]; seprintf(b, lastof(b), "DSWD: tmp.size=%u tmp.c_str=%s", (unsigned int)tmp.size(), tmp.c_str()); diaglog(b); }
	diaglog("DSWD: 4e about to do += literal");
	tmp += PERSONAL_DIR;
	diaglog("DSWD: 5 tmp built");
...
void DetermineBasePaths(const char *exe)
{
	std::string tmp;
	diaglog("DBP: 0 enter");
	diaglog("DBP: 1 before GetHomeDir");
	const std::string homedir = GetHomeDir();
	diaglog("DBP: 2 after GetHomeDir");
#ifdef USE_XDG
	...
	diaglog("DBP: 3 before getcwd");
	char cwd[MAX_PATH];
	if (getcwd(cwd, MAX_PATH) == nullptr) *cwd = '\0';
	diaglog("DBP: 4 after getcwd");

	if (_config_file.empty()) {
		diaglog("DBP: 4a in if cfg empty");
		/* Get the path to working directory of OpenTTD. */
		tmp = cwd;
		diaglog("DBP: 4b after tmp=cwd");
		AppendPathSeparator(tmp);
		diaglog("DBP: 4c after AppendPathSep");
		_searchpaths[SP_WORKING_DIR] = tmp;
		diaglog("DBP: 4d after _searchpaths assign");

		diaglog("DBP: 4e before DoScanWorkingDir");
		_do_scan_working_directory = DoScanWorkingDirectory();
		diaglog("DBP: 4f after DoScanWorkingDir");
	} else {
...
void DeterminePaths(const char *exe, bool only_local_path)
{
	...
	diaglog("DP: 11 after config_dir decl");
	if (!_config_file.empty()) {
		diaglog("DP: 12a config_file not empty");
		config_dir = _searchpaths[SP_WORKING_DIR];
	} else {
		diaglog("DP: 13 before FioFindFullPath");
		std::string personal_dir = FioFindFullPath(BASE_DIR, "openttd.cfg");
		diaglog("DP: 14 after FioFindFullPath");
	...
	diaglog("DP: 15 before Debug misc");
	Debug(misc, 1, "{} found as config directory", config_dir);
	diaglog("DP: 16 after Debug misc");

	_highscore_file = config_dir + "hs.dat";
	diaglog("DP: 17 after _highscore_file");
...
{
	this->subdir = sd;
	diaglog("FS::Scan: 1 enter");

	uint num = 0;

	for (Searchpath sp : _valid_searchpaths) {
		diaglog("FS::Scan: 2 in for loop iter");
		/* Don't search in the working directory */
		if (sp == SP_WORKING_DIR && !_do_scan_working_directory) continue;

		std::string path = FioGetDirectory(sp, sd);
		diaglog("FS::Scan: 3 after FioGetDirectory");
		num += ScanPath(this, extension, path.c_str(), path.size(), recursive);
		diaglog("FS::Scan: 4 after ScanPath");
	}

	if (tars && sd != NO_DIRECTORY) {
		diaglog("FS::Scan: 5 in tars if");
		char tbuf[80]; seprintf(tbuf, lastof(tbuf), "FS::Scan: 5a tar count=%u (SKIPPING)", (unsigned)_tar_filelist[sd].size()); diaglog(tbuf);
		/* AMIPORT: tar loop SKIPPED for openttd debug — _tar_filelist may be corrupt under bebbo-gcc 13.3 codegen issue */
	}

	diaglog("FS::Scan: 6 before switch");
	switch (sd) {
		case BASESET_DIR:
