 ...
 */
/* amiport diaglog -- bisect crash inside GenerateWorld(GWM_EMPTY, 64, 64).
 * Reuse the os_amigaos3.cpp diaglog() singleton to avoid duplicate file
 * handles / O_APPEND races. Symbol is a strong extern "C" defined there. */
extern "C" void diaglog(const char *msg);
static inline void gw_diaglog(const char *m) { diaglog(m); }

void GenerateWorld(GenWorldMode mode, uint size_x, uint size_y, bool reset_settings)
{
	AMIPORT_PROFILE_BEGIN("GenerateWorld");
	gw_diaglog("GW: 1 enter");
	if (HasModalProgress()) {
		gw_diaglog("GW: 1a HasModalProgress -- return early");
		AMIPORT_PROFILE_END("GenerateWorld");
		return;
	}
	_gw.mode   = mode;
	_gw.size_x = size_x;
	...
	gw_diaglog("GW: 2 after SetModalProgress");
	_gw.abort  = false;
	_gw.abortp = nullptr;
	...
	gw_diaglog("GW: 3 after SetLocalCompany");

	InitializeGame(_gw.size_x, _gw.size_y, true, reset_settings);
	gw_diaglog("GW: 4 after InitializeGame");
	PrepareGenerateWorldProgress();
	gw_diaglog("GW: 5 after PrepareGenerateWorldProgress");
	...
	gw_diaglog("GW: 6 after seed init");

	/* Load the right landscape stuff, and the NewGRFs! */
	GfxLoadSprites();
	gw_diaglog("GW: 7 after GfxLoadSprites");
	LoadStringWidthTable();
	gw_diaglog("GW: 8 after LoadStringWidthTable");

	/* Re-init the windowing system */
	ResetWindowSystem();
	gw_diaglog("GW: 9 after ResetWindowSystem");

	/* Create toolbars */
	SetupColoursAndInitialWindow();
	gw_diaglog("GW: 10 after SetupColoursAndInitialWindow");
	SetObjectToPlace(SPR_CURSOR_ZZZ, PAL_NONE, HT_NONE, WC_MAIN_WINDOW, 0);
	gw_diaglog("GW: 11 after SetObjectToPlace");

	UnshowCriticalError();
	gw_diaglog("GW: 12 after UnshowCriticalError");
	CloseAllNonVitalWindows();
	gw_diaglog("GW: 13 after CloseAllNonVitalWindows");
	HideVitalWindows();
	gw_diaglog("GW: 14 after HideVitalWindows");

	ShowGenerateWorldProgress();
	gw_diaglog("GW: 15 after ShowGenerateWorldProgress");

	/* Centre the view on the map */
	if (FindWindowById(WC_MAIN_WINDOW, 0) != nullptr) {
		ScrollMainWindowToTile(TileXY(MapSizeX() / 2, MapSizeY() / 2), true);
		gw_diaglog("GW: 16a after ScrollMainWindowToTile");
	} else {
		gw_diaglog("GW: 16b skipped Scroll (no main window)");
	}

	gw_diaglog("GW: 17 before _GenerateWorld inner");
	_GenerateWorld();
	gw_diaglog("GW: 18 after _GenerateWorld inner -- COMPLETE");
