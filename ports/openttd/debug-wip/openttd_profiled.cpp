...
int openttd_main(int argc, char *argv[])
{
	...
	diaglog("OTTD: F before DeterminePaths");
	DeterminePaths(argv[0], only_local_path);
	diaglog("OTTD: G after DeterminePaths");
	TarScanner::DoScan(TarScanner::BASESET);
	diaglog("OTTD: H after TarScanner BASESET");

	diaglog("OTTD: H1 dedicated check");
	if (dedicated) {
		diaglog("OTTD: H2 dedicated true, SKIPPING Debug()");
		/* Debug(net, 3, "Starting dedicated server, version {}", _openttd_revision); */
		diaglog("OTTD: H3 after Debug-skip");
	}
	diaglog("OTTD: I after Debug net");
	...
	diaglog("OTTD: J before LoadFromConfig");
	LoadFromConfig(true);
	diaglog("OTTD: K after LoadFromConfig");
	...
	diaglog("OTTD: L1 before InitializeLanguagePacks");
	InitializeLanguagePacks();
	diaglog("OTTD: L2 after InitializeLanguagePacks");

	/* Initialize the font cache */
	diaglog("OTTD: L3 before InitFontCache");
	InitFontCache(false);
	diaglog("OTTD: L4 after InitFontCache");

	/* This must be done early, since functions use the SetWindowDirty* calls */
	diaglog("OTTD: L5 before InitWindowSystem");
	InitWindowSystem();
	diaglog("OTTD: L6 after InitWindowSystem");

	diaglog("OTTD: L7 before BaseGraphics::FindSets");
	BaseGraphics::FindSets();
...
void StateGameLoop()
{
	...
	if (_game_mode == GM_EDITOR) {
		...
		BasePersistentStorageArray::SwitchMode(PSM_ENTER_GAMELOOP);
		{ AMIPORT_PROFILE_BEGIN("SGL:AnimateAnimatedTiles"); AnimateAnimatedTiles();   AMIPORT_PROFILE_END("SGL:AnimateAnimatedTiles"); }
		{ AMIPORT_PROFILE_BEGIN("SGL:IncreaseDate");         IncreaseDate();           AMIPORT_PROFILE_END("SGL:IncreaseDate"); }
		{ AMIPORT_PROFILE_BEGIN("SGL:RunTileLoop");          RunTileLoop();            AMIPORT_PROFILE_END("SGL:RunTileLoop"); }
		{ AMIPORT_PROFILE_BEGIN("SGL:CallVehicleTicks");     CallVehicleTicks();       AMIPORT_PROFILE_END("SGL:CallVehicleTicks"); }
		{ AMIPORT_PROFILE_BEGIN("SGL:CallLandscapeTick");    CallLandscapeTick();      AMIPORT_PROFILE_END("SGL:CallLandscapeTick"); }
		BasePersistentStorageArray::SwitchMode(PSM_LEAVE_GAMELOOP);

#ifndef DEBUG_DUMP_COMMANDS
		{
			PerformanceMeasurer framerate(PFE_ALLSCRIPTS);
			{ AMIPORT_PROFILE_BEGIN("SGL:AI::GameLoop");     AI::GameLoop();           AMIPORT_PROFILE_END("SGL:AI::GameLoop"); }
			{ AMIPORT_PROFILE_BEGIN("SGL:Game::GameLoop");   Game::GameLoop();         AMIPORT_PROFILE_END("SGL:Game::GameLoop"); }
		}
#endif
		{ AMIPORT_PROFILE_BEGIN("SGL:UpdateLandscapingLimits"); UpdateLandscapingLimits(); AMIPORT_PROFILE_END("SGL:UpdateLandscapingLimits"); }

		{ AMIPORT_PROFILE_BEGIN("SGL:CallWindowGameTickEvent"); CallWindowGameTickEvent(); AMIPORT_PROFILE_END("SGL:CallWindowGameTickEvent"); }
		{ AMIPORT_PROFILE_BEGIN("SGL:NewsLoop");                NewsLoop();                AMIPORT_PROFILE_END("SGL:NewsLoop"); }
		cur_company.Restore();
	}
...
void GameLoop()
{
	AMIPORT_PROFILE_BEGIN("GameLoop");
	...
	{ AMIPORT_PROFILE_BEGIN("GL:ProcessAsyncSaveFinish"); ProcessAsyncSaveFinish(); AMIPORT_PROFILE_END("GL:ProcessAsyncSaveFinish"); }
	if (gl_first) diaglog("GL: 5 after ProcessAsyncSaveFinish");
	gl_count++;

	/* autosave game? */
	if (_do_autosave) {
		AMIPORT_PROFILE_BEGIN("GL:DoAutosave");
		DoAutosave();
		_do_autosave = false;
		SetWindowDirty(WC_STATUS_BAR, 0);
		AMIPORT_PROFILE_END("GL:DoAutosave");
	}
	...
	if (_switch_mode != SM_NONE && !HasModalProgress()) {
		...
		AMIPORT_PROFILE_BEGIN("GL:SwitchToMode");
		SwitchToMode(_switch_mode);
		AMIPORT_PROFILE_END("GL:SwitchToMode");
		_switch_mode = SM_NONE;
	}
	if (gl_first) diaglog("GL: 8 after switch_mode check");

	{ AMIPORT_PROFILE_BEGIN("GL:IncreaseSpriteLRU"); IncreaseSpriteLRU(); AMIPORT_PROFILE_END("GL:IncreaseSpriteLRU"); }
	...
	if (!_pause_mode && HasBit(_display_opt, DO_FULL_ANIMATION)) {
		AMIPORT_PROFILE_BEGIN("GL:DoPaletteAnimations");
		DoPaletteAnimations();
		AMIPORT_PROFILE_END("GL:DoPaletteAnimations");
	}

	{ AMIPORT_PROFILE_BEGIN("GL:SoundDriverMainLoop"); SoundDriver::GetInstance()->MainLoop(); AMIPORT_PROFILE_END("GL:SoundDriverMainLoop"); }
	{ AMIPORT_PROFILE_BEGIN("GL:MusicLoop"); MusicLoop(); AMIPORT_PROFILE_END("GL:MusicLoop"); }
	if (gl_first) diaglog("GL: 15 GameLoop complete -- first tick done!");

	/* Periodic profiling dump every 100 frames */
#ifdef __AMIGA__
	if ((gl_count % 100) == 0 && gl_count > 0) {
		amiport_profile_dump();
	}
#endif
