...
void VideoDriver_SDL_Default::Paint()
{
	...
	AMIPORT_PROFILE_BEGIN("VideoDriver_SDL_Default::Paint");

	if (IsEmptyRect(this->dirty_rect) && this->local_palette.count_dirty == 0) {
		AMIPORT_PROFILE_END("VideoDriver_SDL_Default::Paint");
		return;
	}
	...
	if (_sdl_surface != _sdl_real_surface) {
		AMIPORT_PROFILE_BEGIN("Paint:SDL_BlitSurface");
		SDL_BlitSurface(_sdl_surface, &r, _sdl_real_surface, &r);
		AMIPORT_PROFILE_END("Paint:SDL_BlitSurface");
	}
#ifdef AMIPORT_PROFILE
	/* Track dirty rect total pixels per second so we know if title menu is
	 * marking the FULL window dirty every frame or just small portions.
	 * Dump to file every 100 calls. */
	{
		extern "C" {
			#include <proto/dos.h>
		}
		static unsigned long dirty_total_pixels = 0;
		static unsigned long dirty_call_count = 0;
		static unsigned long dirty_max_pixels = 0;
		dirty_total_pixels += (unsigned long)(r.w * r.h);
		dirty_call_count++;
		if ((unsigned long)(r.w * r.h) > dirty_max_pixels) dirty_max_pixels = (unsigned long)(r.w * r.h);
		if ((dirty_call_count % 100) == 0) {
			char buf[256];
			int n = snprintf(buf, sizeof(buf),
				"calls=%lu avg_pixels=%lu max_pixels=%lu (1.0=full 640x480=307200)\n",
				dirty_call_count,
				dirty_total_pixels / dirty_call_count,
				dirty_max_pixels);
			BPTR fh = Open((CONST_STRPTR)"WORK:OpenTTD-SDL2/dirty-rect.log", MODE_NEWFILE);
			if (fh) { Write(fh, (CONST_APTR)buf, (LONG)n); Close(fh); }
		}
	}
#endif
	{
		AMIPORT_PROFILE_BEGIN("Paint:SDL_UpdateWinSurfRects");
		SDL_UpdateWindowSurfaceRects(this->sdl_window, &r, 1);
		AMIPORT_PROFILE_END("Paint:SDL_UpdateWinSurfRects");
	}
