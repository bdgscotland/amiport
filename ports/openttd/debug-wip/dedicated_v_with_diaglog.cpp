...
void VideoDriver_Dedicated::MainLoop()
{
	...
	diaglog("DV: 2 dedicated flags set");

	/* If SwitchMode is SM_LOAD_GAME, it means that the user used the '-g' options */
	if (_switch_mode != SM_LOAD_GAME) {
		diaglog("DV: 3 calling StartNewGameWithoutGUI");
		StartNewGameWithoutGUI(GENERATE_NEW_SEED);
		diaglog("DV: 4 StartNewGameWithoutGUI returned");
	} else {
		diaglog("DV: 3a SM_LOAD_GAME path -- no StartNewGameWithoutGUI");
	}

	/* amiport: clear _switch_mode so the first GameLoop tick doesn't
	 * call SwitchToMode -> MakeNewGame which crashes inside the dummy
	 * dedicated video driver path. World was already generated above. */
	_switch_mode = SM_NONE;
	diaglog("DV: 4a _switch_mode cleared to SM_NONE");

	this->is_game_threaded = false;
	diaglog("DV: 5 is_game_threaded = false set");

	/* Done loading, start game! */
	diaglog("DV: 6 entering main while-loop (server is up!)");

	int loop_count = 0;
	while (!_exit_game) {
		bool first = (loop_count == 0);
		if (first) diaglog("DV: 7a iter 0 enter");
		/* amiport: skip DedicatedHandleKeyInput on AmigaOS -- it uses
		 * select()/poll() on stdin which crashes (Guru #80000006 CHK).
		 * AmigaOS console input is via Read()/WaitForChar() and we don't
		 * need interactive console commands for headless server testing.
		 * Re-enable later via WaitForChar(Input(), 0) wrapper. */
#ifndef __AMIGA__
		if (!_dedicated_forks) DedicatedHandleKeyInput();
#endif
		if (first) diaglog("DV: 7b after DedicatedHandleKeyInput-SKIP");
		this->DrainCommandQueue();
		if (first) diaglog("DV: 7c after DrainCommandQueue");

		ChangeGameSpeed(_ddc_fastforward);
		if (first) diaglog("DV: 7d after ChangeGameSpeed");
		this->Tick();
		if (first) diaglog("DV: 7e after Tick");
		this->SleepTillNextTick();
		if (first) diaglog("DV: 7f after SleepTillNextTick (iter 0 done)");
		if (loop_count == 1) diaglog("DV: 7g iter 1 done");
		if (loop_count == 100) diaglog("DV: 7h iter 100 done (server alive!)");
		loop_count++;
	}
	diaglog("DV: 8 MainLoop exiting cleanly");
}
