...
{
	size_t size;
	{ char b[200]; seprintf(b, lastof(b), "MD5: 1 enter file=%s", this->filename); diaglog(b); }
	FILE *f = FioFOpenFile(this->filename, "rb", subdir, &size);
	{ char b[100]; seprintf(b, lastof(b), "MD5: 2 fopen %s size=%u", f?"OK":"NULL", (unsigned)size); diaglog(b); }

	if (f == nullptr) return CR_NO_FILE;
	diaglog("MD5: 3 file open ok");
	...
	diaglog("MD5: 4 entering fread loop");
	...
	{ char b[100]; seprintf(b, lastof(b), "MD5: 5 loop done iters=%d", diaglog_count); diaglog(b); }
	FioFCloseFile(f);

	diaglog("MD5: 6 before Finish");
	checksum.Finish(digest);
	diaglog("MD5: 7 after Finish");
