...
/* amiport: AmigaOS — always use bundled queue.h/tree.h; no util.h/pty.h */
#ifdef __AMIGA__
# include	"queue.h"
# include	"tree.h"
# include	<signal.h>             /* sig_atomic_t */
# include	<amiport/err.h>        /* amiport: err/errx/warn/warnx/strtonum */
# include	<amiport/stdlib.h>     /* exit() → amiport_exit(), getenv → amiport_getenv */
# include	<amiport/unistd.h>     /* POSIX unistd shim */
# include	<amiport/sys/stat.h>   /* stat/fstat shim */
# include	<amiport/string.h>     /* reallocarray → amiport_reallocarray */
# include	<amiport/dirent.h>     /* opendir/readdir/closedir/mkdir shims */
# include	<amiport/termios.h>    /* tcgetattr/tcsetattr → SetMode() */
/* amiport: uid_t/gid_t already defined by libnix sys/types.h — no redefinition needed */
/* amiport: struct timespec already defined by libnix sys/_timespec.h — no redefinition needed */
/* amiport: mode_t not in libnix sys/types.h — define it */
# ifndef _MODE_T_DEFINED
typedef unsigned int mode_t;
#  define _MODE_T_DEFINED
# endif
/* amiport: struct winsize stub — no TIOCGWINSZ on AmigaOS */
...
# define pledge(p, e) (0)
# define unveil(p, f) (0)
/* amiport: popen/pclose — gzip not supported on AmigaOS (isgzip always returns 0) */
# ifndef pclose
...
#if !defined(HAVE_FUTIMENS) && !defined(__AMIGA__)
/* amiport: futimens defined as macro in fileio.c on AmigaOS */
...
#if !defined(__AMIGA__) && (defined(__APPLE__) || defined(__NetBSD__))
#define st_atim st_atimespec
#define st_ctim st_ctimespec
#define st_mtim st_mtimespec
#endif
/* amiport: AmigaOS struct stat has no st_atim/st_mtim — stub via struct timespec in b_fi */
...
int		 compile(int, int);
#else
static int globalwd(void) { return FALSE; } /* amiport: removed inline — C89 compat */
#endif
