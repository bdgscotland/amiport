...
lrealloc(struct line *lp, int newsize)
{
	...
	if (lp->l_size < newsize) {
		/* amiport: add 16-byte headroom to avoid realloc per typed char */
		int alloc = newsize + 16;
		if ((tmp = realloc(lp->l_text, alloc)) == NULL)
			return (FALSE);
		lp->l_text = tmp;
		lp->l_size = alloc;
	}
...
ldelete(RSIZE n, int kflag)
{
	...
	while (n != 0) {
		...
		sv[end] = '\0';
		/* amiport: memmove replaces byte loop — 2-4x faster on 68k */
		memmove(cp1, cp1 + chunk,
		    (size_t)(dotp->l_used - doto - chunk));
