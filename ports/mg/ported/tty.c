...
/* amiport: no sys/ioctl.h on AmigaOS — TIOCGWINSZ stubbed below */
/* amiport: sig_atomic_t from signal.h needed — use int */
typedef int sig_atomic_t;      /* amiport: sig_atomic_t stub */
#include <stdio.h>
#include <string.h>
#else
#include <sys/ioctl.h>
...
ttinit(void)
{
	...
	char *tty;
#ifndef __AMIGA__
	/* amiport: struct sigaction and sigaction() not available on AmigaOS.
	 * SIGWINCH does not exist — AmigaOS has no window resize signal.
	 * winch_flag will remain 0; ttresize() uses 80x24 defaults. */
	struct sigaction sa;
	memset(&sa, 0, sizeof(sa));
	sa.sa_handler = winchhandler; /* amiport: was designated init — C89 compat */
#endif
	...
	/* amiport: on AmigaOS, SIGWINCH/SIGCONT not available — no-op */
#endif
...
ttresize(void)
{
...
#ifdef __AMIGA__
	/* amiport: TIOCGWINSZ not available. Use AmigaOS Window Bounds Report
	 * (CSI 0 SP q -> CSI row1;col1;row2;col2 r) to get real window size.
	 * Falls through to 80x24 default if query fails. */
	{
		char rbuf[32];
		int i = 0, bot = 0, right = 0;
		/* Send Window Status Request */
		Write(Output(), "\x9B" "0 q", 4);
		/* Read response: CSI 1;1;rows;cols r */
		if (WaitForChar(Input(), 500000)) {
			int n = Read(Input(), rbuf, sizeof(rbuf) - 1);
			if (n > 0) {
				int vals[4] = {0,0,0,0};
				int vi = 0;
				rbuf[n] = '\0';
	...
	if ((newrow <= 0 || newcol <= 0) &&
		...
		newrow = 24;  /* amiport: default 80x24 for Amiga */
		newcol = 80;
	}
