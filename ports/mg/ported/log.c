...
#ifdef __AMIGA__
#include <amiport/sys/stat.h>  /* amiport: stat shim */
#include <amiport/unistd.h>    /* amiport: open/close/read/write */
#include <amiport/stdlib.h>    /* amiport: exit → amiport_exit */
/* amiport: sys/queue.h bundled as queue.h (included via def.h) */
#else
#include <sys/queue.h>
#include <sys/stat.h>
#include <fcntl.h>
...
{
	struct undo_rec	*rec;
	static char		 buf[4096], tmp[1024]; /* amiport: static — 5KB on stack risks overflow on 68k */
	int      	 num;
	char		*jptr;
...
mgloginit(void)
{
...
#ifdef __AMIGA__
	/* amiport: umask not meaningful on AmigaOS */
	oumask = 0;
	f_mode = 0777;
	dir_mode = 0777;
#else
	...
	if(stat(mglogdir, &sb)) {
		if (amiport_mkdir(mglogdir, dir_mode) != 0) /* amiport: replaced mkdir() */
			return (FALSE);
