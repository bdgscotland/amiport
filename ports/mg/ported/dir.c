...
#include <amiport/unistd.h>    /* amiport: getcwd/chdir/mkdir */
...
do_makedir(char *path)
{
...
#ifdef __AMIGA__
	/* amiport: umask/chmod not meaningful on AmigaOS — use full permissions */
	oumask = 0;
	f_mode = 0777;
	dir_mode = 0777;
#else
	...
	for (;;) {
		...
		*slash = '\0';
		...
		if (amiport_mkdir(path, finished ? f_mode : dir_mode) == 0) { /* amiport: replaced mkdir() */
#ifndef __AMIGA__
			...
			if (!ishere || !S_ISDIR(sb.st_mode)) {
				...
				/* amiport: umask() is no-op on AmigaOS */
#ifndef __AMIGA__
				umask(oumask);
#endif
				return (FALSE);
	...
	eerase();
#ifndef __AMIGA__
	umask(oumask); /* amiport: no umask on AmigaOS */
#endif
	return (TRUE);
}
