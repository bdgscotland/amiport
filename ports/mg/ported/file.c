...
/* amiport: <libgen.h> dirname() available in libnix but modifies input buffer
 * (crash-patterns #18). xdirname() below passes a strdup() copy — safe.
 * Include libgen.h on AmigaOS too for dirname(). */
#include <libgen.h>
#else
#include <sys/stat.h>
#include <libgen.h>
