...
/* amiport: stack cookie for AmigaOS 3.x with bebbo-gcc.
 * 256KB (not 1MB like OS4/MorphOS) — OS3 systems have limited RAM and the
 * ITEST harness spawns multiple shells. 256KB is sufficient for syntax
 * highlighting, deep eval, and Vim9 on 68k. */
...
/* amiport: Lock() wrapper that suppresses "Please insert volume" requesters.
 * AmigaDOS pops a system requester when Lock() references a non-existent volume
 * (e.g., bare name "vim" becomes "vim:" volume request). pr_WindowPtr = -1
 * is the documented way to disable this (see ADCD dos.library/ErrorReport). */
    static BPTR
...
{
    /* amiport: Delay() is declared in proto/dos.h; local prototype removed */
...
mch_init(void)
{
    ...
    /* amiport: suppress ALL "Please insert volume" system requesters.
     * Vim probes many paths during startup ($VIM, $VIMRUNTIME, defaults.vim,
     * vimrc, etc.). Any path that resolves to a bare name triggers AmigaDOS
     * Lock()/Open() volume requesters via libnix, not just our os_amiga.c code.
     * A CLI editor should never pop up system requesters — handle errors
     * gracefully instead. pr_WindowPtr = -1 is the ADCD-documented approach. */
    {
	struct Process *me = (struct Process *)FindTask(NULL);
	me->pr_WindowPtr = (APTR)-1L;
    }
...
exit:
    if (nilfh)			    // amiport: close NIL: handle to prevent leak
	Close(nilfh);
#ifdef FEAT_ARP
    if (ArpBase)
...
#else
    /* amiport: no gethostname on AmigaOS 3.x / libnix */
    vim_strncpy(s, (char_u *)"amiga", len - 1);
#endif
}
