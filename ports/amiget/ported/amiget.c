/*
 * amiget.c -- Package manager for classic AmigaOS 3.x
 *
...
static const char *verstag = "$VER: amiget 1.0 (05.04.2026)";
long __stack = 65536;
...
static struct amiget_config cfg;
static struct amiget_package manifest[AMIGET_MAX_PACKAGES];
static int manifest_count = 0;
static int manifest_loaded = 0;
static int use_color = 0;

#define CACHE_PATH "S:amiget-cache.json"
...
static int fetch_manifest(void)
{
    ...
    if (rc != 0) {
        fprintf(stderr, "amiget: cannot fetch package list from %s\n",
                cfg.server);
        return -1;
    }
    return 0;
...
static int load_manifest(void)
{
    ...
    manifest_count = amiget_parse_manifest(CACHE_PATH, manifest,
                                            AMIGET_MAX_PACKAGES);
    if (manifest_count < 0) {
    ...
    if (manifest_count >= AMIGET_MAX_PACKAGES) {
        fprintf(stderr, "amiget: warning: package list truncated "
                "at %d entries\n", AMIGET_MAX_PACKAGES);
    }
...
static struct amiget_package *find_package(const char *name)
{
    int i;
...
static int do_install(struct amiget_package *pkg)
{
    char url[512];
    char tmppath[128];
    char destpath[128];
    ...
    if (rc != 0) {
        fprintf(stderr, "amiget: download failed\n");
        remove(tmppath);
        return -1;
    }
    ...
    if (pkg->machine_sha256[0]) {
        ...
        if (strcmp(hash, pkg->machine_sha256) != 0) {
            ...
            fprintf(stderr, "amiget: download corrupted "
                    "(expected %.8s..., got %.8s...)\n",
    ...
    if (sys_rc != 0) {
        fprintf(stderr, "FAILED\n");
        fprintf(stderr, "amiget: extraction failed (lha rc=%ld)\n", sys_rc);
        remove(tmppath);
        return -1;
    }
    fprintf(stderr, "OK\n");
             ...
             cfg.installpath, pkg->name);
    {
        ...
        if (!lock) {
            fprintf(stderr, "amiget: binary not found at %s after extraction\n",
                    destpath);
            remove(tmppath);
            return -1;
        }
    ...
    /* Update DB */
    if (amiget_db_save(pkg->name, pkg->version, destpath) != 0) {
        fprintf(stderr, "amiget: warning: could not update "
                "package database\n");
    }
...
{
    printf("amiget - Package manager for AmigaOS 3.x\n\n");
    printf("Usage: amiget <command> [arguments]\n\n");
    printf("Commands:\n");
    ...
    printf("\nConfig: S:amiget.conf  Database: S:amiget.db\n");
    printf("Server: %s\n", cfg.server);
}
...
static int cmd_list(void)
{
    ...
    for (i = 0; i < manifest_count; i++) {
        const struct amiget_installed *inst;
        const char *tag = "";

        inst = amiget_db_find(manifest[i].name);
        if (inst) {
...
static int cmd_search(const char *term)
{
    ...
    if (!term || !*term) {
        fprintf(stderr, "amiget: search requires a term\n");
        return 10;
    }
...
{
    struct amiget_package pkg;
    char url[256];
    char tmppath[] = "T:amiget-info.json";
    int http_status;
    int rc;
    int i;

    if (!name || !*name) {
        fprintf(stderr, "amiget: info requires a package name\n");
        return 10;
    }
    ...
    if (rc != 0) {
        ...
        struct amiget_package *cached;
        if (load_manifest() < 0) {
            fprintf(stderr, "amiget: cannot reach server or cache\n");
            return 10;
        }
        cached = find_package(name);
        if (!cached) {
            fprintf(stderr, "amiget: package '%s' not found\n", name);
            return 10;
        }
        printf("(cached data - run 'amiget list' to refresh)\n\n");
        color_on(COL_AMBER);
        printf("  %-16s %s\n", "Name:", cached->name);
        color_off();
    ...
    rc = amiget_parse_package_info(tmppath, &pkg, stdout);
    remove(tmppath);
    ...
    {
        const struct amiget_installed *inst = amiget_db_find(pkg.name);
        if (inst) {
...
static int cmd_install(const char *name)
{
    ...
    if (!name || !*name) {
        fprintf(stderr, "amiget: install requires a package name\n");
        return 10;
    }
    ...
    if (!pkg) {
        fprintf(stderr, "amiget: package '%s' not found. "
                "Run 'amiget list' to refresh.\n", name);
        return 10;
    }

    /* Check if already installed */
    inst = amiget_db_find(name);
    ...
    /* Check lha exists */
    {
        ...
        if (!lock) {
            fprintf(stderr, "amiget: C:lha not found - "
                    "install lha first\n");
            return 10;
        }
        UnLock(lock);
    ...
    /* Check requires (warn only) */
    {
        ...
        for (i = 0; i < pkg->num_requires; i++) {
            ...
            if (lib) {
                ...
                fprintf(stderr, "amiget: warning: %s not found "
                        "(may be required)\n", pkg->requires[i]);
            }
        }
    }
...
{
    struct amiget_installed entries[AMIGET_MAX_INSTALLED];
    int count;
    int i;
    int outdated = 0;
    ...
    if (name && *name) {
        ...
        /* Block self-update */
        if (strcmp(name, "amiget") == 0) {
            fprintf(stderr, "amiget: self-update not supported in v1. "
        ...
        inst = amiget_db_find(name);
        if (!inst) {
            fprintf(stderr, "amiget: '%s' is not installed\n", name);
            return 10;
        }

        pkg = find_package(name);
        if (!pkg) {
            fprintf(stderr, "amiget: '%s' not in package list\n", name);
            return 10;
        }
    ...
    count = amiget_db_load(entries, AMIGET_MAX_INSTALLED);
    if (count <= 0) {
    ...
    /* List outdated */
    for (i = 0; i < count; i++) {
        struct amiget_package *pkg = find_package(entries[i].name);
        if (pkg && strcmp(entries[i].version, pkg->version) != 0) {
            if (strcmp(entries[i].name, "amiget") == 0) continue;
            if (outdated == 0) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (pkg && strcmp(entries[i].version, pkg->version) != 0) {
            ...
            printf("\nUpgrading %s %s -> %s\n",
                   entries[i].name, entries[i].version, pkg->version);
            if (do_install(pkg) != 0) {
                fprintf(stderr, "amiget: upgrade of %s failed, "
                        "stopping\n", entries[i].name);
                return 10;
            }
        }
...
static int cmd_remove(const char *name)
{
    ...
    if (!name || !*name) {
        fprintf(stderr, "amiget: remove requires a package name\n");
        return 10;
    }
    ...
    if (!prefix_ok) {
        fprintf(stderr, "amiget: refusing to delete %s "
    ...
    if (!DeleteFile((CONST_STRPTR)inst->path)) {
        ...
        fprintf(stderr, "amiget: warning: file already deleted: %s\n",
                inst->path);
    }
#else
    remove(inst->path);
    ...
    /* Remove from DB */
    if (amiget_db_remove(name) != 0) {
        fprintf(stderr, "amiget: warning: could not update database\n");
    }
...
static int cmd_doctor(void)
{
    ...
    if (all_ok) {
        char url[256];
        char tmppath[] = "T:amiget-doctor.tmp";
        int http_status = 0;
        int rc;
...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(argv[1], "help") == 0 || strcmp(argv[1], "--help") == 0 ||
        ...
        fprintf(stderr, "amiget: unknown command '%s'\n", argv[1]);
        fprintf(stderr, "Run 'amiget help' for usage.\n");
        rc = 10;
    }
