/*
 * json.c -- amiget manifest JSON parser
 *
...
static int parse_requires(const char **pp, struct amiget_package *pkg)
{
    const char *p = *pp;
...
static const char *parse_package(const char *p, struct amiget_package *pkg)
{
    char key[64];
...
int amiget_parse_manifest(const char *path,
                          struct amiget_package *pkgs, int max_pkgs)
{
    FILE *fp;
...
int amiget_parse_package_info(const char *path,
