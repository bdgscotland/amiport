...
amiget is the first package manager for classic 68k AmigaOS. It downloads, verifies (SHA-256), and installs software packages from the amiport repository at amiport.platesteel.net. 9 CLI commands cover the full package lifecycle: discover, install, upgrade, remove, diagnose.
...
| installed | List from S:amiget.db | 0 |
...
4. **Dual-format LHA** — machine-installable (C/<name>) for amiget, Aminet format for humans.
5. **Path prefix safety** — remove only deletes within configured install path.
6. **S:amiget.conf** — optional KEY=VALUE config for server URL, install path, color.
...
make -C ports/amiget           # Build amiget
```
...
- No `amiget upgrade amiget` (self-update deferred to v2)
...
- Config file at S:amiget.conf with KEY=VALUE format
- Package DB at S:amiget.db with space-delimited records
