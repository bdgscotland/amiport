...
**Real fix attempts (all failed, all deferred to a future revision):**

1. **Patch libgit2's `dos_drive_prefix_length` + amigit slash-injection rewrite.** Make `dos_drive_prefix_length` scan for the first `:` on AmigaOS (`#ifdef __AMIGA__`) and accept multi-char drive prefixes; have `amigit_resolve_repo_path` step 2 inject `/` after the volume separator so libgit2 sees `"Ram Disk:/foo"` instead of `"Ram Disk:foo"`. Tried 2026-04-13. Result: in-repo open paths (status/log/diff/add/commit/etc.) regressed from 81/81 to 62/82 because libnix does NOT treat `"Ram Disk:/foo"` the same way as `"Ram Disk:foo"` for `p_stat`/`p_open` operations -- libnix can't open the same file given the two forms. The deeper fix needs both libgit2 path recognition AND libnix path normalization to agree, which is a multi-day effort.

2. **chdir-then-init dance.** `amiport_chdir` into the target directory first, then call `git_repository_init(repo, ".", is_bare)` so libgit2 sees `.` and supposedly uses CWD-relative ops for the .git/ subdir creation. Tried 2026-04-13. Result: same `failed to make directory './.'` error. libgit2 absolutizes `"."` early via `realpath` and ends up with the same multi-char volume form, hitting the same `git_fs_path_root` → -1 → './.' dirname-walk derail. The chdir does not bypass libgit2's path-root limitation.

3. **Manual init bypass via libnix mkdir + fopen.** Bypass `git_repository_init` entirely for the multi-char case by creating the .git/ directory structure directly: `mkdir(.git)`, `mkdir(.git/refs)`, `fopen(.git/HEAD)`, `fputs("ref: refs/heads/master\n")`, etc. Builds a minimal repository that `git_repository_open_ext` can open afterwards. Tried 2026-04-13. Result: the binary built clean but caused unrelated test 29 (`amigit show -h`) to hang on FS-UAE. Root cause not diagnosed in-session; possibly a `<amiport/dirent.h>` header-include side effect, a binary-size pressure on the test harness, or an amiport_mkdir interaction. Reverted to avoid shipping a regression. See `~/.claude/projects/-Users-duncan-Developer-amiport/memory/gap_amigit_test29_hang.md` for the investigation TODO.

Until one of these (or a fourth approach) can be made to work, the friendly-error precheck in `ports/amigit/ported/cmd_init.c` is the working compromise. amigit 0.1-3 (2026-04-13) catches BOTH the bare-CWD-on-multi-char-volume case AND the explicit-multi-char-volume-path case (`amigit init WORK:foo`) by testing the resolved-path shape (`resolved[1] != ':' && strchr(resolved, ':') != NULL`) rather than the input-path shape.
